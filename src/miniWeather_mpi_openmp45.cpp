...
#pragma omp target data map(                                                   \
    to                                                                         \
    : state_tmp[:(nz + 2 * hs) * (nx + 2 * hs) * NUM_VARS], hy_dens_cell       \
    [:nz + 2 * hs], hy_dens_theta_cell                                         \
    [:nz + 2 * hs], hy_dens_int                                                \
    [:nz + 1], hy_dens_theta_int                                               \
    [:nz + 1], hy_pressure_int                                                 \
    [:nz + 1]) map(alloc                                                       \
                   : flux[:(nz + 1) * (nx + 1) * NUM_VARS], tend               \
                   [:nz * nx * NUM_VARS], sendbuf_l                            \
                   [:hs * nz * NUM_VARS], sendbuf_r                            \
                   [:hs * nz * NUM_VARS], recvbuf_l                            \
                   [:hs * nz * NUM_VARS], recvbuf_r                            \
                   [:hs * nz * NUM_VARS])                                      \
    map(tofrom                                                                 \
...
#pragma omp target teams distribute parallel for simd collapse(2) private(     \
    stencil, vals, d3_vals) depend(inout                                       \
                                   : asyncid) nowait
  for (int k = 0; k < nz; k++) {
...
#pragma omp target update from(                                                \
    sendbuf_l[:nz * hs * NUM_VARS], sendbuf_r[:nz * hs * NUM_VARS])            \
    depend(inout                                                               \
           : asyncid) nowait
#pragma omp taskwait
...
#pragma omp target update to(                                                  \
    recvbuf_l[:nz * hs * NUM_VARS], recvbuf_r[:nz * hs * NUM_VARS])            \
    depend(inout                                                               \
           : asyncid) nowait
...
void output(double *state, double etime) {
  ...
  // 利用 openmp 处理 netcdf 的 I/O 操作
#pragma omp target update from(                                                \
    state[:(nz + 2 * hs) * (nx + 2 * hs) * NUM_VARS]) depend(inout             \
                                                             : asyncid) nowait
#pragma omp taskwait
